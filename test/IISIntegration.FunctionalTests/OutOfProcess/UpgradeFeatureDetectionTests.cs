...
namespace Microsoft.AspNetCore.Server.IISIntegration.FunctionalTests
{
    public class UpgradeFeatureOutOfProcessDetectionTests : LoggedTest
    {
        ...
        {
            return UpgradeFeatureDetectionDeployer(RuntimeFlavor.CoreClr, ApplicationType.Portable, "AppHostConfig/WebsocketsNotSupported.config", Helpers.GetOutOfProcessTestSitesPath(), "Disabled");
        }

        [Fact]
        public Task UpgradeFeatureDetectionDisabled_OutOfProcess_IISExpress_CoreClr_x64_Portable()
        {
            return UpgradeFeatureDetectionDeployer(RuntimeFlavor.CoreClr, ApplicationType.Portable, "AppHostConfig/Http.config", Helpers.GetOutOfProcessTestSitesPath(), "Enabled");
        }

        private async Task UpgradeFeatureDetectionDeployer(RuntimeFlavor runtimeFlavor, ApplicationType applicationType, string configPath, string sitePath, string expected)
        {
